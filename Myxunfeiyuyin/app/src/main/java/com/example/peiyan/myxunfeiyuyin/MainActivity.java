...
public class MainActivity extends AppCompatActivity {
            ...
            public void onResult(RecognizerResult recognizerResult, boolean b) {
               ...
               System.out.println("解析后的数据 "+ content);
                ...
                if (b) {
                    ...
                    String anwser = "耳膜被震破了";
                    if (result.contains("你好")) {
                        anwser = "你好,我是你的智能语音助手,很高兴为你服务";
                    } else if (result.contains("小炮")) {
                        anwser = "和小炮一块玩";
                    } else if (result.contains("美女")) {
                        String[] anwserList = new String[]{"帅哥哥，一起玩好嘛"};
    ...
    private String parseData(String resultString) {
        ...
        RootBean xfBean = gson.fromJson(resultString, RootBean.class);
        //创建集合,用来存放bean类里的对象
        ArrayList<RootBean.WS> ws = xfBean.ws;
        //创建一个容器,用来存放从每个集合里拿到的数据,使用StringBUndle效率高
        StringBuilder stringBuilder = new StringBuilder();
        for (RootBean.WS w : ws) {
